...
export default function Home() {
  ...
  const brandColor = company.primaryColor || "#a05c32";
  ...
  return <div className="min-h-screen bg-[#fbfaf7] text-[#29251f]">
    <header className="sticky top-0 z-30 border-b border-[#e9e3d9]/80 bg-[#fbfaf7]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
        <div className="flex items-center gap-3">
          {company.logoUrl ? <img src={company.logoUrl} alt={company.name} className="h-10 w-10 rounded-2xl object-cover" /> : <div className="brand-mark" style={{ backgroundColor: brandColor }}><Utensils className="h-5 w-5" /></div>}
          <div><p className="font-display text-xl font-bold leading-none tracking-tight">{company.name}</p><p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#a06b3b]">{company.tagline || "comida de verdade"}</p></div>
        </div>
    ...
    <main>
      ...
      <section id="historia" className="mx-auto max-w-7xl px-5 py-18 lg:px-8"><div className="grid gap-10 rounded-[28px] bg-[#f1ece3] p-8 sm:p-12 lg:grid-cols-[1fr_1fr] lg:items-center"><div><p className="section-kicker">Nossa cozinha</p><h2 className="mt-3 max-w-md font-display text-4xl font-bold leading-tight">Comida honesta, bonita e sem pressa.</h2><p className="mt-5 max-w-lg leading-7 text-[#776e63]">{company.name} nasceu para resgatar o almoço que tem textura, aroma e história. Ingredientes simples, técnica cuidadosa e um cardápio que muda com o que está mais bonito na feira.</p><Button variant="outline" className="mt-7 rounded-full border-[#cdbfae] bg-transparent text-[#2f5d50] hover:bg-white">Conheça nosso jeito <ArrowRight className="ml-2 h-4 w-4" /></Button></div><div className="story-photo"/></div></section>
    </main>
    <footer className="border-t border-[#e9e3d9] px-5 py-8 lg:px-8"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 text-sm text-[#776e63] sm:flex-row"><p>© 2026 {company.name}. Feita para alimentar bem.</p><p>Seg a sex · 11h às 14h30 · <span className="text-[#a05c32]">(11) 98888-0000</span></p></div></footer>
