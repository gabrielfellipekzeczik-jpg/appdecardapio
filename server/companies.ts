...
export async function updateCompanyBranding(companyId: number, input: { name?: string; tagline?: string; logoUrl?: string; heroImageUrl?: string; primaryColor?: string; templateId?: "classic" | "modern" | "cover" | "premium"; pickupAddress?: string }) {
  const db = await getDb();
  if (!db) return;
